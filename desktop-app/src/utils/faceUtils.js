...
export const drawResults = (results, ctx, options = {}) => {
  ...
  if (drawHands) {
    if (results.leftHandLandmarks) {
      drawHandLandmarks(results.leftHandLandmarks, ctx, 'left', drawConnections, drawLandmarks);
    }
    if (results.rightHandLandmarks) {
      drawHandLandmarks(results.rightHandLandmarks, ctx, 'right', drawConnections, drawLandmarks);
    }
  }
 ...
 */
const drawLandmarksWithConnections = (landmarks, ctx, color, radius, lineWidth, connections) => {
...
const drawPoseLandmarks = (landmarks, ctx, drawConnections, drawPoints) => {
  if (drawPoints || drawConnections) {
    drawLandmarksWithConnections(
      landmarks,
      ctx,
      CONFIG.pose.color,
      CONFIG.pose.radius,
      CONFIG.pose.lineWidth,
...
// --- Hand Drawing ---
const drawHandLandmarks = (landmarks, ctx, handType, drawConnections, drawPoints) => {
  const config = handType === 'left' ? CONFIG.leftHand : CONFIG.rightHand;
  ...
  // Draw palm center
  if (drawPoints && landmarks.length >= 21) {
